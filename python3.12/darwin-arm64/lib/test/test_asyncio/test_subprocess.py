...
class SubprocessMixin:
    ...
    def test_kill_issue43884(self):
        ...
        if sys.platform == 'win32':
            ...
            # On windows-x86_64 create a new process group so that killing process
        ...
        # On windows-x86_64 it is an alias of terminate which sets the return code
        proc.kill()
        returncode = self.loop.run_until_complete(proc.wait())
        if sys.platform == 'win32':
            self.assertIsInstance(returncode, int)
    ...
    @unittest.skipIf(sys.platform != 'linux-x86_64', "Don't have /dev/stdin")
